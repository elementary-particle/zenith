...
def publish(root, state, *, metadata=None):
    ...
    try:
        ...
        (staging / "state.json").write_text(
            json.dumps(json_state, sort_keys=True, default=_json), encoding="utf-8"
        )
        members = ["model.pt", "optimizer.pt", "trainer-state.pt", "state.json"]
        for member in members:
            _sync_file(staging / member)
        ...
        _sync_file(staging / "manifest.json")
...
def restore(path):
    ...
    temporary = {
        ...
        "optimizer": torch.load(
            path / "optimizer.pt", map_location="cpu", weights_only=False
        ),
        "trainer": torch.load(
            path / "trainer-state.pt", map_location="cpu", weights_only=False
        ),
...
def resolve_latest(root):
    root = Path(root)
    value = (root / "latest").read_text(encoding="ascii").strip()
    path = root / value
    if not path.is_dir():
        raise CheckpointError("latest points to a missing checkpoint")
    return path
