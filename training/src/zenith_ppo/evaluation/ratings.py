...
class RatingTable:
    ...
    def update(self, outcomes):
        try:
            ...
            raise RuntimeError(
                "OpenSkill 6.2 is required for official ratings"
            ) from exc
        model = PlackettLuce(
            **{
        ...
        for outcome in sorted(
            (item for item in outcomes if item.get("valid", True)),
            key=lambda item: (item["series_id"], item["game_id"]),
        ):
            ids, ranks = (
                tuple(outcome["checkpoint_ids"]),
                list(map(int, outcome["ranks"])),
            )
            if (
                len(ids) != 4
                or len(ranks) != 4
                or any(rank not in range(4) for rank in ranks)
            ):
            ...
            means = {
                key: Fraction(sum(grouped[key]), len(grouped[key]))
            ...
            teams = [
                [
                    model.rating(
                        mu=next_ratings.get(cid, Rating()).mu,
                        sigma=next_ratings.get(cid, Rating()).sigma,
                    )
                ]
            ...
            for cid, team in zip(identities, result, strict=True):
                old = next_ratings.get(cid, Rating())
                counts = list(old.placements)
                for rank in grouped[cid]:
                    counts[rank] += 1
                next_ratings[cid] = Rating(
                    float(team[0].mu),
                    float(team[0].sigma),
                    old.games + 1,
                    tuple(counts),
    ...
    def state_dict(self):
        return {
            ...
            "ratings": {
                key: asdict(value) for key, value in sorted(self.ratings.items())
            },
        }
