...
def test_next_draw_matches_after_round_trip():
    streams = SeedStreams(10); state = streams.state_dict(); expected = streams.python_rng("minibatch").random()
    restored = SeedStreams(10); restored.load_state_dict(state)
...
def test_global_python_numpy_torch_cpu_and_cuda_states_round_trip():
    ...
    if torch.cuda.is_available(): torch.cuda.set_rng_state_all(states[3])
