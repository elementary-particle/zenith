...
def validate_win_action(obs: Observation, action: Action,
    ...
    if is_ron:
        ...
        shanten = calculate_shanten(hand_13)
        assert shanten == 0, (
...
def validate_action_in_legals(obs: Observation, action: Action, *, ctx: str) -> None:
...
def validate_mjai_roundtrip(obs: Observation, action: Action, *, ctx: str) -> None:
...
def validate_first_obs_scores(kyoku, first_obs: Observation, *, ctx: str) -> None:
...
def validate_tenhou_log(log_path: Path, *, rule: str | None = None) -> None:
    ...
    Checks:
      ...
      - Mask / legal_actions consistency
