...
/// Returns the versioned on-disk table path used by the process.
pub fn configured_cache_path() -> PathBuf {
    shanten_table::configured_cache_path()
}

/// Generates and atomically writes the shanten decomposition table.
pub fn build_cache(path: &Path) -> io::Result<()> {
    shanten_table::build_cache(path)
}

/// Builds the configured cache when it is missing or invalid.
pub fn ensure_cache() -> io::Result<PathBuf> {
    shanten_table::ensure_cache()
}
...
pub fn improving_tiles(counts: &[u8; 34], open_melds: u8, visible: &[u8; 34]) -> [u8; 34] {
    let baseline = calculate(counts, open_melds).overall;
    let mut result = [0; 34];
    for i in 0..34 {
        if counts[i] + visible[i] < 4 {
            let mut next = *counts;
            next[i] += 1;
            if calculate(&next, open_melds).overall < baseline {
                result[i] = 4 - counts[i] - visible[i];
            }
        }
    }
    result
