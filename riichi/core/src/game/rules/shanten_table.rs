...
/// A repository-owned decomposition table for standard-hand shanten.
///
/// Every bit records an achievable `(melds, taatsu, pair)` decomposition for
/// one base-5 encoded suit or honor histogram. Keeping the generated data on
/// disk avoids embedding a large opaque asset in the extension module.
...
/// Returns the configured cache path. `ZENITH_SHANTEN_CACHE` overrides the
/// XDG/Home defaults and is useful for containers and shared training hosts.
pub fn configured_cache_path() -> PathBuf {
...
/// Builds a fresh cache at `path`, replacing an older compatible or corrupt
/// file atomically.
pub fn build_cache(path: &Path) -> io::Result<()> {
    ShantenTable::generate().write(path)
}

/// Ensures the configured cache is readable and returns its path.
pub fn ensure_cache() -> io::Result<PathBuf> {
    let path = configured_cache_path();
    if ShantenTable::read(&path).is_err() {
        build_cache(&path)?;
    }
    Ok(path)
}
    ...
    TABLE
        .get_or_init(|| {
            ...
            ShantenTable::read(&path).unwrap_or_else(|_| {
                ...
                // A read-only installation remains usable; prewarming via the
                // builder CLI is recommended for multi-process training.
