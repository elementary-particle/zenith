...
use crate::evaluator::agari;
use crate::evaluator::score;
use crate::evaluator::types::{Conditions, Hand, Meld, MeldType, WinResult, Wind};
use crate::evaluator::yaku;
#[cfg(any())]
use pyo3::prelude::*;

#[cfg_attr(any(), pyclass)]
...
impl HandEvaluator {
    ...
    pub fn is_tenpai(&self) -> bool {
        ...
        let mut hand_14 = self.hand.clone();
        for i in 0..crate::evaluator::types::TILE_MAX {
    ...
    pub fn get_waits_u8(&self) -> Vec<u8> {
        ...
        for i in 0..crate::evaluator::types::TILE_MAX {
            if hand_14.counts[i] < 4 {
                hand_14.add(i as u8);
                if crate::evaluator::agari::is_agari(&mut hand_14) {
                    waits.push(i as u8);
                }
                hand_14.remove(i as u8);
            }
...
pub fn check_riichi_candidates(tiles_136: Vec<u8>) -> Vec<u32> {
    ...
    for (i, &t_discard) in tiles_136.iter().enumerate() {
        let mut hand = crate::evaluator::types::Hand::default();
