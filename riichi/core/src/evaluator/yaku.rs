...
use crate::evaluator::agari::{self, Division, Mentsu};
use crate::evaluator::types::{Hand, Meld};
...
#[derive(Debug, Clone)]
#[cfg_attr(any(), pyclass(get_all, from_py_object))]
pub struct Yaku {
    pub id: u32,
    pub name: String,
    pub name_en: String,
    ...
    for div in &divisions {
        ...
        for wg_idx in win_group_indices {
            ...
            for (i, &t) in yakuhai_tiles.iter().enumerate() {
                let count = div
                    ...
                    + melds
                        .iter()
                        .filter(|m| {
                            m.tiles[0] == t && m.meld_type != crate::evaluator::types::MeldType::Chi
                        })
                        .count();
                if count > 0 {
                    res.han += count as u8;
            ...
            let haku_koutsu = div.body.iter().any(|m| match m {
                ...
                .iter()
                .any(|m| m.tiles[0] == 31 && m.meld_type != crate::evaluator::types::MeldType::Chi);
            let hatsu_koutsu = div.body.iter().any(|m| match m {
                Mentsu::Koutsu(t) => *t == 32,
                _ => false,
            }) || melds
                .iter()
                .any(|m| m.tiles[0] == 32 && m.meld_type != crate::evaluator::types::MeldType::Chi);
            let chun_koutsu = div.body.iter().any(|m| match m {
                Mentsu::Koutsu(t) => *t == 33,
                _ => false,
            }) || melds
                .iter()
                .any(|m| m.tiles[0] == 33 && m.meld_type != crate::evaluator::types::MeldType::Chi);
            ...
            let koutsu_total = div
                ...
                + melds
                    .iter()
                    .filter(|m| m.meld_type != crate::evaluator::types::MeldType::Chi)
                    .count();
            if koutsu_total == 4 {
                res.han += 2;
            ...
            for m in melds {
                if m.meld_type == crate::evaluator::types::MeldType::Ankan {
                    closed_koutsu_count += 1;
                }
            }
            ...
            let kantsu_count = melds
                .iter()
                .filter(|m| {
                    m.meld_type == crate::evaluator::types::MeldType::Daiminkan
                        || m.meld_type == crate::evaluator::types::MeldType::Ankan
                        || m.meld_type == crate::evaluator::types::MeldType::Kakan
                })
                .count();
            if kantsu_count == 3 {
                res.han += 2;
...
fn calculate_fu_with_waiting(
    ...
    for m in melds {
        if m.tiles.len() >= 3 && m.tiles[0] == m.tiles[1] {
            ...
            if m.meld_type == crate::evaluator::types::MeldType::Daiminkan
                || m.meld_type == crate::evaluator::types::MeldType::Ankan
                || m.meld_type == crate::evaluator::types::MeldType::Kakan
            {
                f *= 4;
...
fn check_pinfu(
    ...
    if let Some(Mentsu::Shuntsu(t)) = wg_idx.map(|idx| div.body[idx]) {
...
fn apply_yakuman(
    ...
    for w in 27..=30 {
        let has_koutsu = div.body.iter().any(|m| match m {
            ...
            .iter()
            .any(|m| m.tiles[0] == w && m.meld_type != crate::evaluator::types::MeldType::Chi);
        if has_koutsu {
            wind_koutsu_count += 1;
        } else if div.head == w {
            wind_pair_count += 1;
...
fn check_ittsu(div: &Division, melds: &[Meld]) -> bool {
    for suit_offset in [0, 9, 18] {
        ...
        // Check Melds
        for m in melds {
            if m.meld_type == crate::evaluator::types::MeldType::Chi {
                let t = m.tiles[0];
                if t == suit_offset {
                    has_123 = true;
                } else if t == suit_offset + 3 {
